...
const fechTableKeywords = async (businessSearchCriteria: {
  ...
  if (pageId === -1) {
            ...
            logger.log({
              ...
              if (AlltotalRowCount > 1000) {
                ...
                for (let index = 0; index < calculator; index++) {
                  ...
                  logger.log({
                    level: 'info',
                    message: `The total business is ${AlltotalRowCount}`,
                    calculator: `The total number of pages is ${calculator}`,
    ...
    for (let index = 0; index < jsonKeywords.length; index++) {
      logger.log({
        level: 'debug',
        message: `Number of bussness processed is ${businessIdContainer.length}`
      })
